...
const OrgIcon = ({ org }: { org: string }) => {
  switch (org) {
    case 'ICANN': return <Server className="text-blue-400" size={32} />;
    case 'APNIC': return <Network className="text-emerald-400" size={32} />;
    case 'IGF': return <Globe2 className="text-purple-400" size={32} />;
    case 'SECURITY': return <ShieldAlert className="text-red-400" size={32} />;
    default: return <AlertTriangle className="text-amber-400" size={32} />;
  }
};
      ...
      <div className="grid grid-cols-1 gap-4">
        {scenario.options.map((option) => (
          <button
            ...
            className="group relative flex items-center bg-slate-800 hover:bg-slate-750 border border-slate-600 hover:border-blue-400 rounded-lg p-5 text-left transition-all duration-200 hover:shadow-lg hover:shadow-blue-500/10 hover:-translate-y-0.5"
          >
